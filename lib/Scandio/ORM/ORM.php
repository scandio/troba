...
class ORM extends PDOWrapper
{
    /**
     * Constant for inner join used by ORM::join()
     */
    const INNER_JOIN = 'INNER JOIN';

    /**
     * Constant for left join used by ORM::join()
     */
    const LEFT_JOIN = 'LEFT JOIN';

    /**
     * Constant for right join used by ORM::join()
     */
    const RIGHT_JOIN = 'RIGHT JOIN';

    /**
     * Constant for query type used by ORM::query()
     */
    ...
    /**
     * Constant for convention handler used as key in ORM::initialize()
     */
    const CONVENTION_HANDLER = 'convention_handler';

    /**
     * Constant for sql builder used as key in ORM::initialize()
     */
     ...
     *                  ORM::RUN_MODE => [ORM::DEV_MODE | ORM::PROD_MODE]
     *                  ORM::RESULT_SET_CLASS => '<a class that implements ResultSetInterface>'
     *                  ORM::CONVENTION_HANDLER => <object that implements ConventionHandlerInterface>
     *                  ORM::SQL_BUILDER => <object that implements SqlBuilderInterface>
     *              ]
     *
    ...
    public static function initialize($config = [], $connectionName = 'default')
    {
        ...
        if (array_key_exists(static::CONVENTION_HANDLER, $config)
            ...
            static::$conventionHandler[$connectionName] = $config[ORM::CONVENTION_HANDLER];
        } else {
    ...
    public static function update($object)
    {
        ...
        $primaryQuery = static::primaryQuery($object, null, 'orm_key_');
        ...
        foreach (static::tableMeta($objectOrClass)->getPrimary() AS $primary) {
            ...
            if (is_null($params)) {
                ...
                if ($queryParams[$prefix . $primary] == null) {
                    throw new ORMException('null is not a valid parameter for a primary query', 9003);
                }
            } else if ($isNotAssoc) {
    ...
    public static function queryOneByPrimary($objectOrClass, $params)
    {
        ...
        ], ORM::QUERY_TYPE_ARRAY);
    }

    /**
     * @param array|string $queryParams depends on the parameter type
     * @param string $paramsType optional default is ORM::QUERY_TYPE_ARRAY
     * @return object
     */
     ...
     * @param string $paramType optional default is ORM::QUERY_TYPE_QUERY
    ...
    public static function query($queryParams = null, $paramType = self::QUERY_TYPE_QUERY)
    {
        if ($paramType == self::QUERY_TYPE_QUERY) {
            ...
            if (is_null($queryParams)) throw new ORMException('Invalid json code', 9001);
        } elseif (!is_array($queryParams)) {
            throw new ORMException('Method requires an array', 9002);
        }
    ...
    {
        if (is_callable([$object, $event])) $object->{$event}();
    }
